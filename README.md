...
1. Người dùng tạo tài khoản qua `/api/users`.
...
| `POST /api/users` | `identity-service` | Tạo tài khoản |
| `/api/plans/**` | `work-service` | Quản lý plans |
| `/api/tasks/**` | `work-service` | Quản lý tasks |
| `/api/dashboard/**` | `work-service` | Tổng quan dashboard |
...
| `POST /tasks` | Tạo task trong một plan |

Lưu ý: `TaskServiceImpl` đã có logic create/read/update/delete task theo quyền sở hữu.
...
- Đồng bộ hoàn toàn frontend React với các REST endpoint chuẩn của backend.
